...
pub async fn create_deployment(
	...
	if body.name.trim().is_empty() || body.endpoint_path.trim().is_empty() {
		return Err(bad_request("deployment name and endpoint path are required"));
	}
...
pub async fn update_deployment(
	...
	let traffic_split = normalize_traffic_split(
		&strategy_type,
		body.traffic_split.unwrap_or(current_split),
	)
	.map_err(bad_request)?;
