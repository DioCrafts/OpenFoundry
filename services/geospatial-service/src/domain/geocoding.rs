...
const KNOWN_LOCATIONS: [(&str, Coordinate); 7] = [
	("madrid", Coordinate { lat: 40.4168, lon: -3.7038 }),
	("barcelona", Coordinate { lat: 41.3874, lon: 2.1686 }),
	("paris", Coordinate { lat: 48.8566, lon: 2.3522 }),
	("berlin", Coordinate { lat: 52.52, lon: 13.405 }),
	("lisbon", Coordinate { lat: 38.7223, lon: -9.1393 }),
	("london", Coordinate { lat: 51.5072, lon: -0.1276 }),
	("new york", Coordinate { lat: 40.7128, lon: -74.006 }),
];

pub fn forward(address: &str) -> GeocodeResponse {
	let normalized = address.trim().to_lowercase();
	if let Some((label, coordinate)) = KNOWN_LOCATIONS.iter().find(|(label, _)| normalized.contains(label)) {
