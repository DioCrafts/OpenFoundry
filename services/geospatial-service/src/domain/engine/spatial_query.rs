use crate::models::{
	...
	spatial_index::{SpatialOperation, SpatialQueryRequest, SpatialQueryResponse, SpatialQuerySummary},
};
...
pub fn execute(layer: &LayerDefinition, request: &SpatialQueryRequest) -> SpatialQueryResponse {
	let mut features = match request.operation {
		...
		SpatialOperation::Nearest => nearest_query(layer, request.point, request.limit.unwrap_or(5)),
		SpatialOperation::Buffer => buffer_query(layer, request.point, request.radius_km.unwrap_or(10.0)),
	};

	let nearest_distance_km = if matches!(request.operation, SpatialOperation::Nearest) {
		request
			.point
			.and_then(|point| features.first().map(|feature| point.distance_km(feature.geometry.centroid())))
	} else {
		None
	};
...
fn nearest_query(layer: &LayerDefinition, point: Option<Coordinate>, limit: usize) -> Vec<MapFeature> {
...
fn buffer_query(layer: &LayerDefinition, point: Option<Coordinate>, radius_km: f64) -> Vec<MapFeature> {
	let Some(point) = point else {
		return Vec::new();
	};
...
fn buffer_ring(center: Coordinate, radius_km: f64) -> Vec<Coordinate> {
	...
	vec![
		Coordinate { lat: center.lat + delta, lon: center.lon },
		Coordinate { lat: center.lat + delta / 2.0, lon: center.lon + delta },
		Coordinate { lat: center.lat - delta / 2.0, lon: center.lon + delta },
		Coordinate { lat: center.lat - delta, lon: center.lon },
		Coordinate { lat: center.lat - delta / 2.0, lon: center.lon - delta },
		Coordinate { lat: center.lat + delta / 2.0, lon: center.lon - delta },
