...
pub async fn create_layer(
	...
	let features = serde_json::to_value(&request.features).map_err(|cause| internal_error(cause.to_string()))?;
...
pub async fn update_layer(
	...
	let mut layer = LayerDefinition::try_from(existing).map_err(|cause| internal_error(cause.to_string()))?;
	...
	let now = Utc::now();
	let style = serde_json::to_value(&layer.style).map_err(|cause| internal_error(cause.to_string()))?;
	let features = serde_json::to_value(&layer.features).map_err(|cause| internal_error(cause.to_string()))?;
	let tags = serde_json::to_value(&layer.tags).map_err(|cause| internal_error(cause.to_string()))?;
