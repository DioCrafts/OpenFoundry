...
fn key_based_pairs(records: &[EntityRecord], strategy: &BlockingStrategyConfig) -> Vec<CandidatePair> {
	...
	for (key, group) in groups {
		for left_index in 0..group.len() {
			for right_index in left_index + 1..group.len() {
				emit_pair(&mut pairs, &mut emitted, &key, group[left_index], group[right_index]);
			}
		}
	}
...
fn lsh_pairs(records: &[EntityRecord], strategy: &BlockingStrategyConfig) -> Vec<CandidatePair> {
	...
	for (bucket, group) in buckets {
		for left_index in 0..group.len() {
			for right_index in left_index + 1..group.len() {
				emit_pair(&mut pairs, &mut emitted, &bucket, group[left_index], group[right_index]);
			}
		}
	}
...
fn blocking_key(record: &EntityRecord, key_fields: &[String]) -> String {
	...
	if parts.is_empty() {
		parts.push(normalize_text(&record.display_name).chars().take(6).collect());
	}
