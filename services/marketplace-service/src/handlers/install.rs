...
use crate::{
	...
	handlers::{bad_request, db_error, internal_error, load_installs, load_listing_row, load_versions, not_found, ServiceResult},
	models::{install::{CreateInstallRequest, InstallRecord}, ListResponse},
	AppState,
};

pub async fn list_installs(State(state): State<AppState>) -> ServiceResult<ListResponse<InstallRecord>> {
	let installs = load_installs(&state.db).await.map_err(|cause| db_error(&cause))?;
...
pub async fn create_install(
	...
	let versions = load_versions(&state.db, request.listing_id).await.map_err(|cause| db_error(&cause))?;
