...
use crate::{
	...
	handlers::{db_error, load_listing_row, load_listings, load_reviews, load_versions, not_found, ServiceResult},
...
pub async fn get_overview(State(state): State<AppState>) -> ServiceResult<MarketplaceOverview> {
	let listings = load_listings(&state.db).await.map_err(|cause| db_error(&cause))?;
...
pub async fn list_categories(State(state): State<AppState>) -> ServiceResult<ListResponse<CategoryDefinition>> {
	let listings = load_listings(&state.db).await.map_err(|cause| db_error(&cause))?;
...
pub async fn list_listings(State(state): State<AppState>) -> ServiceResult<ListResponse<ListingDefinition>> {
	let listings = load_listings(&state.db).await.map_err(|cause| db_error(&cause))?;
...
pub async fn get_listing(
	...
	let listing = ListingDefinition::try_from(row).map_err(|cause| crate::handlers::internal_error(cause.to_string()))?;
