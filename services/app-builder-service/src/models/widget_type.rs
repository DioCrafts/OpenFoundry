...
pub fn widget_catalog() -> Vec<WidgetCatalogItem> {
	vec![
		WidgetCatalogItem {
			...
			description: "Paginated records from a dataset, ontology object list, or SQL query.".to_string(),
			category: "data".to_string(),
			default_props: json!({ "page_size": 10, "striped": true }),
			default_size: WidgetDefaultSize { width: 8, height: 5 },
			supported_bindings: vec!["dataset".to_string(), "query".to_string(), "ontology".to_string()],
		...
		WidgetCatalogItem {
			...
			default_size: WidgetDefaultSize { width: 6, height: 5 },
			supported_bindings: vec!["dataset".to_string(), "ontology".to_string(), "static".to_string()],
			supports_children: false,
		},
		WidgetCatalogItem {
			widget_type: "chart".to_string(),
			label: "Chart".to_string(),
			description: "Line, bar, pie, or area chart backed by query or dataset bindings.".to_string(),
			category: "visualization".to_string(),
			default_props: json!({ "chart_type": "line", "x_field": "label", "y_field": "value" }),
			default_size: WidgetDefaultSize { width: 6, height: 4 },
		...
		WidgetCatalogItem {
			...
			default_size: WidgetDefaultSize { width: 4, height: 2 },
		...
		WidgetCatalogItem {
			...
			default_size: WidgetDefaultSize { width: 4, height: 3 },
		...
		WidgetCatalogItem {
			...
			description: "Calls event handlers such as navigate, filter, open link, or execute query.".to_string(),
			category: "actions".to_string(),
			default_props: json!({ "label": "Run action", "variant": "primary" }),
			default_size: WidgetDefaultSize { width: 3, height: 1 },
			supported_bindings: vec!["static".to_string(), "ontology".to_string()],
			supports_children: false,
		},
		WidgetCatalogItem {
			...
			default_size: WidgetDefaultSize { width: 12, height: 3 },
