...
pub fn collector_catalog(events: &[AuditEvent]) -> Vec<CollectorStatus> {
	...
	services
		...
		.map(|(service, subject, connected)| {
			let service_events = events.iter().filter(|event| event.source_service == *service).count();
			CollectorStatus {
				...
				last_event_at: events
					...
					.max(),
				backlog_depth: if service_events == 0 { 2 } else { (service_events % 5) as i32 },
				health: if service_events > 0 { "healthy".to_string() } else { "warming".to_string() },
