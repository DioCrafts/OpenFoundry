...
use crate::models::{
	...
	compliance_report::{GdprEraseResponse, GdprExportPayload, GdprExportRequest, GdprEraseRequest},
};
...
pub fn erase_response(request: &GdprEraseRequest, events: &[AuditEvent]) -> GdprEraseResponse {
	...
	GdprEraseResponse {
		...
		status: if request.hard_delete { "scheduled-redaction".to_string() } else { "masked".to_string() },
