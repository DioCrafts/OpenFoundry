...
#[tokio::main]
async fn main() {
    observability::init_tracing("audit-service");
    ...
    let public = Router::new().route(
        "/health",
        get(|| async { axum::Json(HealthStatus::ok("audit-service")) }),
    );
    ...
    let protected = Router::new()
        ...
        .route(
            ...
            axum::routing::post(handlers::reports::export_subject_data),
        )
        .route(
            "/api/v1/audit/gdpr/erase",
            axum::routing::post(handlers::reports::erase_subject_data),
        )
