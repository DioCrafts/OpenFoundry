...
pub fn build_authorization_url(
	...
	authorize_url.query_pairs_mut()
		...
		.append_pair("client_id", provider.client_id.as_deref().unwrap_or_default())
...
pub async fn exchange_code(
	...
	let response = Client::new()
		...
		.form(&[
			...
			("client_id", provider.client_id.as_deref().unwrap_or_default()),
			("client_secret", provider.client_secret.as_deref().unwrap_or_default()),
		])
		.send()
		.await
...
pub fn map_identity(provider: &SsoProvider, payload: &Value) -> Result<(String, String, String), String> {
