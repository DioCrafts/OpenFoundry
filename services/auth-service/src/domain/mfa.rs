...
pub fn verify_totp(secret: &str, code: &str) -> bool {
	...
	(-1_i64..=1_i64).any(|offset| generate_totp(secret, offset).is_some_and(|candidate| candidate == code))
}

pub fn hash_recovery_codes(codes: &[String]) -> Value {
	json!(codes.iter().map(|code| security::hash_token(code)).collect::<Vec<_>>())
}
...
pub fn issue_challenge(config: &JwtConfig, user: &User, auth_method: &str) -> Result<String, auth_middleware::JwtError> {
	let claims = Claims {
		sub: user.id,
...
pub fn validate_challenge(config: &JwtConfig, token: &str) -> Result<Claims, auth_middleware::JwtError> {
	let claims = jwt::decode_token(config, token)?;
	if claims.token_use.as_deref() == Some("mfa_challenge") {
		Ok(claims)
	} else {
		Err(auth_middleware::JwtError::Invalid("invalid challenge token".to_string()))
	}
}
