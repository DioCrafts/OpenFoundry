...
pub fn evaluate_text(content: &str) -> GuardrailVerdict {
	...
	for token in content.split_whitespace() {
		...
		let numeric_count = token.chars().filter(|character| character.is_ascii_digit()).count();
	...
	let blocked = flags.iter().any(|flag| flag.severity == "high" && flag.kind != "pii_email" && flag.kind != "pii_phone");
...
fn looks_like_email(token: &str) -> bool {
	token.contains('@')
		&& token.split('@').count() == 2
		&& token.split('@').last().map(|value| value.contains('.')).unwrap_or(false)
}
