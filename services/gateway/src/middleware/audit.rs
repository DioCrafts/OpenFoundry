	...
	if let Ok(nats_url) = std::env::var("NATS_URL") {
		...
		tokio::spawn(async move {
			match event_bus::connect(&nats_url).await {
				Ok(js) => {
					if let Err(cause) = subscriber::ensure_stream(&js, streams::AUDIT, &[subjects::AUDIT]).await {
				...
				Err(cause) => tracing::warn!(?cause, "failed to connect to NATS for audit publishing"),
			}
		});
	}
