...
pub fn posture(
	...
	let encrypted_in_transit = sync_status.map(|status| status.encrypted_in_transit).unwrap_or(true);
	let encrypted_at_rest = sync_status.map(|status| status.encrypted_at_rest).unwrap_or(true);
