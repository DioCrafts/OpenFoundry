...
pub fn build_plans(
	...
	shares
		.iter()
		.map(|share| {
			let sync_status = sync_statuses.iter().find(|status| status.share_id == share.id);
			let compatibility = compatibility.iter().find(|report| report.share_id == share.id);
			let encrypted = sync_status
				.map(|status| status.encrypted_in_transit && status.encrypted_at_rest)
				.unwrap_or(false);
			let status = if compatibility.map(|report| report.compatible).unwrap_or(false) {
				sync_status
			...
			ReplicationPlan {
				...
				status,
				rows_replicated: sync_status.map(|status| status.rows_replicated).unwrap_or_default(),
				backlog_rows: sync_status.map(|status| status.backlog_rows).unwrap_or_default(),
