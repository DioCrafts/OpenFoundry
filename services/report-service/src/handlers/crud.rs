...
pub async fn create_report(
	...
	let template = serde_json::to_value(&request.template).map_err(|cause| internal_error(cause.to_string()))?;
...
pub async fn update_report(
	...
	let mut report = ReportDefinition::try_from(existing_row).map_err(|cause| internal_error(cause.to_string()))?;
	...
	let now = Utc::now();
	let template = serde_json::to_value(&report.template).map_err(|cause| internal_error(cause.to_string()))?;
	let schedule_value = serde_json::to_value(&report.schedule).map_err(|cause| internal_error(cause.to_string()))?;
	let recipients = serde_json::to_value(&report.recipients).map_err(|cause| internal_error(cause.to_string()))?;
	let tags = serde_json::to_value(&report.tags).map_err(|cause| internal_error(cause.to_string()))?;
