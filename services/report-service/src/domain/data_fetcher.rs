...
pub fn build_snapshot(report: &ReportDefinition) -> ReportDataSnapshot {
    let seed =
        (report.name.len() + report.dataset_name.len() + report.template.sections.len()) as i64;
    let highlights = vec![
        ReportHighlight {
...
fn section_snapshot(
    ...
    let summary = match section.kind {
        ...
        SectionKind::Table => format!(
            "{} exposes the ranked regional split for the current cycle.",
            section.title
        ),
        SectionKind::Chart => format!(
            "{} keeps the last four buckets for trend inspection.",
            section.title
        ),
        SectionKind::Narrative => format!(
            "{} captures the analyst narrative for the deck.",
            section.title
        ),
        SectionKind::Map => format!(
            "{} projects geospatial hotspots for executive review.",
            section.title
        ),
    };
