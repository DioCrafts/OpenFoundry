...
use crate::models::{
	recipient::{DistributionChannel, DistributionChannelCatalogEntry, DistributionRecipient, DistributionResult},
...
pub fn catalog() -> Vec<DistributionChannelCatalogEntry> {
	vec![
		...
		DistributionChannelCatalogEntry {
			...
			description: "Persist generated artifacts into object storage for downstream sharing.".to_string(),
...
pub fn simulate_distribution(
	...
	report
		...
		.iter()
		.enumerate()
		.map(|(index, recipient)| delivery_result(recipient, generated_at, report.name.as_str(), index as i64))
		.collect()
}
