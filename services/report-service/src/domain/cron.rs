...
pub fn next_run_at(schedule: &ReportSchedule, now: DateTime<Utc>) -> Option<DateTime<Utc>> {
	...
	match schedule.cadence {
		...
		ScheduleCadence::Cron => Some(now + Duration::minutes(schedule.interval_minutes.unwrap_or(30))),
...
pub fn build_schedule_board(
	...
	let mut upcoming = reports
		.iter()
		.filter_map(|report| {
			report.schedule.next_run_at.or_else(|| next_run_at(&report.schedule, now)).map(|next_run_at| {
				ScheduledRun {
					report_id: report.id,
	...
	ScheduleBoard {
		active_schedules: reports.iter().filter(|report| report.schedule.enabled).count(),
		paused_reports: reports.iter().filter(|report| !report.schedule.enabled).count(),
