...
pub fn simulate_ci_run(repository_id: uuid::Uuid, branch_name: &str, commit: Option<&CommitDefinition>) -> CiRun {
	let now = Utc::now();
	CiRun {
		id: uuid::Uuid::now_v7(),
		repository_id,
		branch_name: branch_name.to_string(),
		commit_sha: commit.map(|entry| entry.sha.clone()).unwrap_or_else(|| "manual-trigger".to_string()),
