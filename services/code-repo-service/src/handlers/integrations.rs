...
pub async fn update_integration(
	...
	let current = load_integration_row(&state.db, id)
		...
		.map_err(|cause| db_error(&cause))?
		.ok_or_else(|| not_found("integration not found"))?;
	let current = RepositoryIntegration::try_from(current).map_err(|cause| internal_error(cause.to_string()))?;
	let branch_mapping = serde_json::to_value(request.branch_mapping.clone().unwrap_or(current.branch_mapping.clone()))
	...
	.bind(id)
	.bind(request.external_namespace.unwrap_or(current.external_namespace))
	...
	.bind(request.ci_trigger_strategy.unwrap_or(current.ci_trigger_strategy))
...
pub async fn trigger_sync(
	...
	let integration_row = load_integration_row(&state.db, id)
		...
		.map_err(|cause| db_error(&cause))?
		.ok_or_else(|| not_found("integration not found"))?;
	let integration = RepositoryIntegration::try_from(integration_row).map_err(|cause| internal_error(cause.to_string()))?;
