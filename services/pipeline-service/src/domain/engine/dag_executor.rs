...
use super::{execute_node, ExecutionRequest, NodeResult};
...
pub async fn execute_pipeline(
	...
	for (stage_index, stage) in stages.into_iter().enumerate() {
		...
		let mut stage_results = stream::iter(stage.into_iter().enumerate())
			.map(move |(position, node_id)| async move {
				let node = node_lookup
					.get(node_id.as_str())
					.copied()
					.ok_or_else(|| format!("pipeline node '{}' not found", node_id))?;
				let mut result = execute_node_with_retries(query_ctx, node, max_attempts).await;
				result.stage_index = Some(stage_index);
				result.worker_id = Some(format!("pipeline-worker-{}", (position % worker_budget) + 1));
...
fn execution_stages(nodes: &[PipelineNode], start_from_node: Option<&str>) -> Result<Vec<Vec<String>>, String> {
	...
	for node in nodes.iter().filter(|node| reachable.contains(&node.id)) {
		...
		for dependency in &node.depends_on {
			...
			adjacency.entry(dependency.as_str()).or_default().push(node.id.as_str());
