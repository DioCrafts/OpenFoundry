...
async fn persist_quality_snapshot(
	...
	let passed_rules = profile.rule_results.iter().filter(|result| result.passed).count() as i32;
...
async fn fetch_numeric_stats(
	...
	let row = rows.first();
	let min_value = row.and_then(|values| values.first()).cloned().filter(|value| value != "null");
	let max_value = row.and_then(|values| values.get(1)).cloned().filter(|value| value != "null");
...
fn normalize_json_bytes(data: &[u8]) -> Result<Vec<u8>, String> {
	...
	if trimmed.starts_with('[') || trimmed.starts_with('{') {
		let parsed: serde_json::Value = serde_json::from_slice(data).map_err(|error| error.to_string())?;
...
async fn collect_rows(ctx: &QueryContext, sql: &str) -> Result<Vec<Vec<String>>, String> {
	let batches = ctx.execute_sql(sql).await.map_err(|error| error.to_string())?;
