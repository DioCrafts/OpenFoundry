				...
				<div class="rounded-[24px] border border-slate-200 bg-slate-50 p-4 dark:border-slate-800 dark:bg-slate-900">
					...
					{#if runtime.latest_run}
						<div class="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-5 text-sm text-slate-700 dark:text-slate-200">
							<div class="rounded-2xl border border-slate-200 bg-white px-3 py-3 dark:border-slate-800 dark:bg-slate-950">In {runtime.latest_run.metrics.input_events}</div>
							<div class="rounded-2xl border border-slate-200 bg-white px-3 py-3 dark:border-slate-800 dark:bg-slate-950">Out {runtime.latest_run.metrics.output_events}</div>
							<div class="rounded-2xl border border-slate-200 bg-white px-3 py-3 dark:border-slate-800 dark:bg-slate-950">P95 {runtime.latest_run.metrics.p95_latency_ms}ms</div>
							<div class="rounded-2xl border border-slate-200 bg-white px-3 py-3 dark:border-slate-800 dark:bg-slate-950">Join {runtime.latest_run.metrics.join_output_rows}</div>
							<div class="rounded-2xl border border-slate-200 bg-white px-3 py-3 dark:border-slate-800 dark:bg-slate-950">CEP {runtime.latest_run.metrics.cep_match_count}</div>
						</div>
						<div class="mt-4 grid gap-4 xl:grid-cols-2">
							<div class="rounded-2xl border border-amber-200 bg-amber-50/70 px-4 py-3 dark:border-amber-900 dark:bg-amber-950/20">
								...
								<p class="mt-2 text-sm text-slate-700 dark:text-slate-200">{runtime.latest_run.backpressure_snapshot.status} • queue {runtime.latest_run.backpressure_snapshot.queue_depth}/{runtime.latest_run.backpressure_snapshot.queue_capacity} • lag {runtime.latest_run.backpressure_snapshot.lag_ms}ms</p>
							</div>
							<div class="rounded-2xl border border-cyan-200 bg-cyan-50/70 px-4 py-3 dark:border-cyan-900 dark:bg-cyan-950/20">
								...
								<p class="mt-2 text-sm text-slate-700 dark:text-slate-200">{runtime.latest_run.state_snapshot.backend} • {runtime.latest_run.state_snapshot.key_count} keys • {runtime.latest_run.state_snapshot.disk_usage_mb}MB</p>
							</div>
						</div>
					{/if}
