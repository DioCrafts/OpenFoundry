...
pub fn generate_spec(proto_dir: &Path) -> Result<OpenApiSpec> {
	...
	let rpc_regex = Regex::new(r"rpc\s+(?P<name>\w+)\((?P<request>[^)]+)\)\s+returns\s+\((?P<response>[^)]+)\)")?;
	...
	for file in proto_files {
		...
		for service_capture in service_regex.captures_iter(&content) {
			let body = service_capture.name("body").map(|value| value.as_str()).unwrap_or_default();
			let rpcs = rpc_regex
				.captures_iter(body)
				.filter_map(|rpc| {
					Some(ProtoRpc {
			...
			if !rpcs.is_empty() {
				services.push(ProtoService {
					...
					name: service_capture.name("name").map(|value| value.as_str()).unwrap_or_default().to_string(),
					rpcs,
				});
			}
		}
			...
			let operation = OpenApiOperation {
				...
				request_body: if method == "get" || method == "delete" {
					...
					Some(OpenApiRequestBody {
						...
						content: BTreeMap::from([(
							...
							OpenApiMediaType { schema: request_ref },
						)]),
					})
				},
				responses: BTreeMap::from([(
					...
					OpenApiResponse {
						...
						content: BTreeMap::from([(
							"application/json".to_string(),
							OpenApiMediaType { schema: response_ref },
						)]),
					},
				)]),
			};

			paths.entry(path).or_insert_with(BTreeMap::new).insert(method, operation);
		}
	}
	...
	Ok(OpenApiSpec {
		...
		info: OpenApiInfo {
			...
			description: "Auto-generated JSON/HTTP contract derived from OpenFoundry proto services.".to_string(),
		},
		paths,
...
fn parse_message_schemas(content: &str) -> Result<BTreeMap<String, OpenApiSchema>> {
	...
	let field_regex = Regex::new(r"(?P<label>repeated\s+)?(?P<type>map<[^>]+>|[a-zA-Z0-9_\.]+)\s+(?P<name>\w+)\s*=\s*\d+")?;
	let mut schemas = BTreeMap::new();

	for capture in message_regex.captures_iter(content) {
		let name = capture.name("name").map(|value| value.as_str()).unwrap_or_default().to_string();
		let body = capture.name("body").map(|value| value.as_str()).unwrap_or_default();
		let mut properties = BTreeMap::new();
		for field in field_regex.captures_iter(body) {
			let field_name = field.name("name").map(|value| value.as_str()).unwrap_or_default().to_string();
			let field_type = field.name("type").map(|value| value.as_str()).unwrap_or_default();
...
fn primitive_or_ref(field_type: &str) -> OpenApiSchema {
	match field_type {
		"string" | "bytes" => OpenApiSchema { schema_type: Some("string".to_string()), format: None, properties: None, items: None, reference: None, additional_properties: None },
		"bool" => OpenApiSchema { schema_type: Some("boolean".to_string()), format: None, properties: None, items: None, reference: None, additional_properties: None },
		"float" | "double" => OpenApiSchema { schema_type: Some("number".to_string()), format: None, properties: None, items: None, reference: None, additional_properties: None },
		"int32" | "uint32" => OpenApiSchema { schema_type: Some("integer".to_string()), format: Some("int32".to_string()), properties: None, items: None, reference: None, additional_properties: None },
		"int64" | "uint64" => OpenApiSchema { schema_type: Some("integer".to_string()), format: Some("int64".to_string()), properties: None, items: None, reference: None, additional_properties: None },
		"google.protobuf.Timestamp" => OpenApiSchema { schema_type: Some("string".to_string()), format: Some("date-time".to_string()), properties: None, items: None, reference: None, additional_properties: None },
