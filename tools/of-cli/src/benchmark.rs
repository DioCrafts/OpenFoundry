...
pub async fn run_suite(scenario_path: &Path, output_path: &Path) -> Result<()> {
	let raw = fs::read_to_string(scenario_path)
		.with_context(|| format!("failed to read benchmark scenario {}", scenario_path.display()))?;
	let resolved = resolve_env_templates(&raw)?;
	let suite: ScenarioSuite = serde_json::from_str(&resolved)
		.with_context(|| format!("failed to parse benchmark scenario {}", scenario_path.display()))?;
	...
	for scenario in &suite.scenarios {
		...
		if statuses.iter().any(|status| *status != scenario.expected_status) {
...
fn resolve_env_templates(raw: &str) -> Result<String> {
	...
	if !missing.is_empty() {
		missing.sort();
		missing.dedup();
		bail!("missing environment variables for benchmark scenario: {}", missing.join(", "));
	}
...
fn default_warmup_iterations() -> usize { 1 }
fn default_measure_iterations() -> usize { 5 }